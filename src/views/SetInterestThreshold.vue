                ...
                <main class="w-full">
                    ...
                    <div class="mb-12 flex flex-row">
                        <div class="w-1/2 mr-4">
                            <div v-bind:class="[{'disabled-container': listHasBeenEditedManually}]" class="slider__wrapper mb-2" @mouseover="showSliderWarning">
                                <span class="block font-subtitle text-white-very-opaque leading-loose mb-4"><h1 class="font-subtitle text-white-opaque inline">1.</h1>How long do you usually spend on a game<br/> before you <strong class="text-white">decide if its a good one</strong></span>

                                <time-slider v-on:sliderChanged="changeThreshold"></time-slider>
                            </div>
                            <span v-if="isSliderWarningVisible" class="slider-warning font-sans bg-orange-dark py-2 px-2"><strong>Warning:</strong> Changing the slider now, will reset your manual list sorting</span>
                        </div>
                        <div class="w-1/2">
                            <span class="block font-subtitle text-white-very-opaque leading-loose mb-4"><h1 class="font-subtitle text-white-opaque inline">2.</h1>Split your game library into <br/> games you <strong class="text-white">like</strong> and <strong class="text-white">dislike</strong></span>
                    ...
                    <section class="w-full flex">
                        <div class="w-1/2 inline-block mr-4">
                            <div class="flex justify-start mb-2">
                                <h3 class="font-subtitle tracking-wider text-white mr-6">DISLIKE</h3>
                                <h4 class="font-subtitle text-white-very-very-opaque">{{ gamesSplit.boring.length }}</h4>
                            </div>

                            <div v-for="boringGame in gamesSplit.boring" class="game__container boring w-32 inline-block relative" :key="boringGame.appid">
                                <div class="inline-block" @click="moveGameIntoList('boring','interesting',boringGame)">
                                    <img alt="game logo" v-lazy="convertSteamImageHashToUrl(boringGame.appid, boringGame.img_logo_url)" />
                                </div>
                                <div class="icon__overlay absolute pin-t pin-l w-full h-full inline-flex justify-center items-center bg-black-opaque">
                                    <p class="text-white text-xs font-sans m-2">{{boringGame.name}}</p>
                                    <img class="icon-like w-12" src="../assets/img/interest/arrow-right.svg"/>
                                </div>
                            </div>
                        ...
                        <div class="w-1/2 inline-block">
                            <div class="flex justify-start mb-2">
                                ...
                                <h4 class="font-subtitle text-white-very-very-opaque ml-6">{{ gamesSplit.interesting.length }}</h4>
                            </div>
                            <div v-for="interestingGame in gamesSplit.interesting" class="game__container interesting w-32 inline-block relative" :key="interestingGame.appid">
                                <div  class="inline-block" @click="moveGameIntoList('interesting','boring',interestingGame)">
                                    <img alt="game logo" v-lazy="convertSteamImageHashToUrl(interestingGame.appid, interestingGame.img_logo_url)" />
                                </div>
                                <div class="icon__overlay absolute pin-t pin-l w-full h-full inline-flex justify-center items-center bg-black-opaque">
                                    <img class="icon-dislike w-12" src="../assets/img/interest/arrow-left.svg"/>
                                    <p class="text-white text-xs font-sans m-2">{{interestingGame.name}}</p>
                                </div>
                            </div>
        ...
        methods:{
            ...
            moveGameIntoList(fromListName,toListName,game){
                ...
                if(!this.gamesSplit[toListName].includes(game)){
                    ...
                    this.gamesSplit[fromListName] = this.removeElementFromArray(this.gamesSplit[fromListName],game);
                }
            ...
            recalculateSplit(gamesToSplit){
                const splitResults = this.splitGamesOnThreshold(this.interestThreshold,gamesToSplit);
