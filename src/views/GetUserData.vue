            ...
            <div class="flex">
                ...
                <main class="w-full md:w-1/2 flex flex-col justify-center items-center">
                    ...
                    <article class="w-full md:w-4/5 flex flex-col items-center mb-6">
                        ...
                        <h1 class="text-white uppercase font-title text-4xl tracking-extra-wide text-center mb-8">Show me<br/> what you play</h1>

                        <div v-if="!isGamesListSet" class="relative w-4/5">
                            <input ref="steamUserID" maxlength="300" v-model="steamUserID" @keyup.enter="findUser(steamUserID)" autofocus
                                   class="w-full h-16 rounded-full text-sm text-center bg-white text-black font-subtitle"
                                   placeholder="SteamID or SteamProfileURL" :class="{'animated flipInX': inputHasBeenTransformed}">
                            <button @click="findUser(steamUserID)" class="button-primary absolute block h-16 w-16 text-xs rounded-full font-bold pin-t pin-r">
                                GO
                            </button>
                    ...
                    <article v-if="isSteamUserQualified()" class="animated zoomIn bg-black-light p-6 w-full md:w-2/3 mb-6">

                        <div class="flex flex-row justify-around items-center mb-6">
                            <img alt="avatar" class="w-16 rounded-full" :src="steamUser.profile.avatarURL">
                            <div class="w-5/6 p-3">
                                <span class="block text-white opacity-50 font-sans mb-2">Find new games for:</span>
                                <p class="text-white font-subtitle break-words mb-3">{{ steamUser.profile.username }}</p>
                                <div v-if="steamUser.games">
                                    <span class="text-white font-subtitle text-sm">{{ steamUser.games.count }}</span>
                                    <span class="text-white opacity-50 font-sans text-lg ml-1">Games</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end">
                            ...
                            <button @click="submitSteamID(steamUser)"
                    ...
                    <meme-message class="self-start w-auto" :key="createIDBasedOnMessageCounter(currentMessageText, messageCounter)" v-if="currentMessageComponent">
                        <component :is="currentMessageComponent" :message="currentMessageText" :steamUserID="steamUserID"></component>
                    </meme-message>
...
<script>
    ...
    import {fetchSteamUserProfile, fetchOwnedGames, saveUserGames, saveUserProfile, fetchSteamIDByUsername} from "../requests/user.js";
    ...
    export default {
        ...
        methods: {
            ...
            isInputProfileURL(userInput){
              return userInput.search(/(\/steamcommunity\.com\/id\/)|(\/steamcommunity\.com\/profiles\/)/) !== -1;
            ...
            fetchSteamIDByCustomizedID(username){
                fetchSteamIDByUsername(username)
                    .then((result) => {
            ...
            fetchAllUserInformationBySteamID(steamID){
                Promise.all(
                    [fetchSteamUserProfile(steamID), fetchOwnedGames(steamID)])
                    .then((result) => {
