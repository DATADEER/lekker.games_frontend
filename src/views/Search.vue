        ...
        <section class="bg-white">
            ...
            <div class="flex">
                <main class="w-full flex flex-wrap">
                    ...
                    <div v-for="app in apps" :key="app.id" v-if="app.name" class="w-full md:w-1/6 rounded overflow-hidden shadow-lg bg-black m-8 md:mt-8 md:mb-8 md:ml-8 md:mr-0 ">
                        <img class="w-full" :src="app.image" alt="Sunset in the mountains">
                        <div class="px-6 py-4">
                            <div class="font-bold text-white text-xl mb-2">{{ app.name }}</div>
                            <p class="text-grey-darker text-base">
                                {{ app.description }}
                            </p>
                        </div>
                        <div class="px-6 py-4">
                            <span v-for="category in app.categories" :key="category.id" class="inline-block bg-grey-lighter rounded-full px-3 py-1 text-sm font-semibold text-grey-darker mr-2 mb-2">{{  category.description }}</span>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                apps:[{id:80},{id:10},{id:393380},{id:271590},{id:586200}]
