<template>
    <div class="timeSlider text-white">
        <div class="range">
            <input :disabled="this.disabled" class="border-0 pb-4 pt-4" v-model="sliderInput" type="range" min="1" max="12" steps="1" value="1">
        </div>

        <ul class="range-labels list-reset flex flex-row justify-between">
            <li class="range-label text-sm" v-for="rangeOption in allRangeOptions" v-bind:key="rangeOption.value"
                v-bind:class="{'active selected': rangeOption.value === selectedValue}"
                @click="selectRangeByLabel(rangeOption, allRangeOptions)">{{ convertMinutesToHours(rangeOption.value) }}<span
                    class="text-xs text-white-very-very-opaque">{{ rangeOption.unit }}</span></li>
        </ul>
    </div>
</template>
